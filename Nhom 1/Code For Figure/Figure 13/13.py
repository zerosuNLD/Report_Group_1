...
alpha = 0.1  # Regularization strength for L1 and L2
...
lasso_model.fit(X_combined_train, Y_train.ravel())
...
Y_predict_lasso_train = lasso_model.predict(X_combined_train)
Y_predict_lasso_test = lasso_model.predict(X_combined_test)
...
mse_lasso_train = mean_squared_error(Y_train, Y_predict_lasso_train)
...
print(f"Lasso Regression MSE - Train: {mse_lasso_train}, Test: {mse_lasso_test}")
...
Y_predict_lasso_plot = lasso_model.predict(x0_combined)
...
axs[0].plot(X_train_scaled, Y_train, 'ro', label='Dữ liệu huấn luyện')
axs[0].plot(X_test_scaled, Y_test, 'bo', label='Dữ liệu kiểm thử')
axs[0].plot(x0, Y_predict_default_plot, label='Hồi quy tuyến tính')
axs[0].plot(x0, Y_predict_optimal_plot, label='Công suất tối ưu', linestyle='--')
axs[0].set_xlim(x_min - x_padding, x_max + x_padding)
axs[0].set_ylim(y_min - y_padding, y_max + y_padding)
axs[0].set_xlabel('X (đã chuẩn hóa)')
axs[0].set_ylabel('Y')
axs[0].set_title('Hồi quy tuyến tính')
axs[0].legend(loc='upper right')
axs[0].grid(True)

# Add MSE values to the plot
textstr_default = f'Hồi quy tuyến tính MSE:\nTrain: {mse_default_train:.2f}\nTest: {mse_default_test:.2f}'
props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
axs[0].text(0.05, 0.95, textstr_default, transform=axs[0].transAxes, fontsize=10,
            verticalalignment='top', bbox=props)

# Plot for Lasso Regression and Optimal Capacity
axs[1].plot(X_train_scaled, Y_train, 'ro', label='Dữ liệu huấn luyện')
axs[1].plot(X_test_scaled, Y_test, 'bo', label='Dữ liệu kiểm thử')
axs[1].plot(x0, Y_predict_lasso_plot, label='Hồi quy Lasso')
axs[1].plot(x0, Y_predict_optimal_plot, label='Công suất tối ưu', linestyle='--')
axs[1].set_xlim(x_min - x_padding, x_max + x_padding)
axs[1].set_ylim(y_min - y_padding, y_max + y_padding)
axs[1].set_xlabel('X (đã chuẩn hóa)')
axs[1].set_ylabel('Y')
axs[1].set_title('Hồi quy Lasso')
axs[1].legend(loc='upper right')
axs[1].grid(True)

# Add MSE values to the plot
textstr_lasso = f'Hồi quy Lasso MSE:\nTrain: {mse_lasso_train:.2f}\nTest: {mse_lasso_test:.2f}'
axs[1].text(0.05, 0.95, textstr_lasso, transform=axs[1].transAxes, fontsize=10,
            verticalalignment='top', bbox=props)

# Plot for Ridge Regression and Optimal Capacity
axs[2].plot(X_train_scaled, Y_train, 'ro', label='Dữ liệu huấn luyện')
axs[2].plot(X_test_scaled, Y_test, 'bo', label='Dữ liệu kiểm thử')
axs[2].plot(x0, Y_predict_ridge_plot, label='Hồi quy Ridge')
axs[2].plot(x0, Y_predict_optimal_plot, label='Công suất tối ưu', linestyle='--')
axs[2].set_xlim(x_min - x_padding, x_max + x_padding)
axs[2].set_ylim(y_min - y_padding, y_max + y_padding)
axs[2].set_xlabel('X (đã chuẩn hóa)')
axs[2].set_ylabel('Y')
axs[2].set_title('Hồi quy Ridge')
axs[2].legend(loc='upper right')
axs[2].grid(True)

# Add MSE values to the plot
textstr_ridge = f'Hồi quy Ridge MSE:\nTrain: {mse_ridge_train:.2f}\nTest: {mse_ridge_test:.2f}'
axs[2].text(0.05, 0.95, textstr_ridge, transform=axs[2].transAxes, fontsize=10,
            verticalalignment='top', bbox=props)

# Hiển thị các trọng số của Lasso và Ridge
fig, ax = plt.subplots(1, 1, figsize=(10, 6))
width = 0.4
ind = np.arange(len(lasso_model.coef_))
ax.bar(ind - width/2, lasso_model.coef_, width, label='Lasso')
ax.bar(ind + width/2, ridge_model.coef_, width, label='Ridge')
ax.set_xlabel('Feature Index')
ax.set_ylabel('Coefficient Value')
ax.set_title('So sánh trọng số Lasso và Ridge')
